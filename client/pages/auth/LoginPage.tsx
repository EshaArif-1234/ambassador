...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (sessionExpired) {
      setErrors((prev) => ({
        ...prev,
        submit: 'Your admin session has expired after 2 hours. Please sign in again.',
      }));
    }
    ...
    <div className="min-h-screen bg-gray-100/70 flex items-center justify-center py-8 px-4 sm:px-6 lg:px-8 relative">
      ...
      {sessionExpired && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 w-full max-w-md px-4">
          <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 text-amber-900 px-4 py-3 rounded-lg shadow-md text-sm">
            <svg className="w-5 h-5 text-amber-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <p>
              <span className="font-semibold">Session expired.</span> Your admin session ended after 2 hours. Please sign in again.
            </p>
          </div>
        </div>
      )}
